...
Spectrum3f PathTracer::radiance(const Scene& scene, RndSampler* sampler, Ray& ray, int max_depth) const {
	Intersection its;
...
void PathTracer::render(const Scene &scene, const RenderOptions &options, ptr<float> rendered_image) const {
...
#pragma omp parallel for
		for (int idx_pixel = block_start; idx_pixel < block_end; idx_pixel++) {
			...
			for (int idx_sample = 0; idx_sample < options.num_samples; idx_sample++) {
				...
				sample_vals[idx_sample] = radiance(scene, &sampler, ray, options.max_bounces);
