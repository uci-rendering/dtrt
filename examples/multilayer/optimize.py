...
scene, integrator = pydtrt.load_mitsuba('./top_view.xml')
...
direc = 'results/'
T1 = SceneTransform("SHAPE_TRANSLATE", torch.tensor([1.0, 0, 0], dtype=torch.float), 4)
T2 = SceneTransform("SHAPE_TRANSLATE", torch.tensor([0, 0.1, 0], dtype=torch.float), 4)
T3 = SceneTransform("SHAPE_TRANSLATE", torch.tensor([0, 0, 1.0], dtype=torch.float), 4)
transform_list = []
...
target_var = torch.tensor([-1.0, -2.5, -1.0])
