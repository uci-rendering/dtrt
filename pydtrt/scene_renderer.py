...
def serialize_scene(scene):
    """
        Given a Pydtrt scene, convert it to a linear list of argument,
...
def render_scene(integrator, options, *args):
    ...
    camera = dtrt.Camera(resolution[0], resolution[1],
                            dtrt.float_ptr(cam_to_world.data_ptr()),
                            dtrt.float_ptr(cam_to_ndc.data_ptr()),
                            clip_near, cam_med_id, 
                            dtrt.float_ptr(cam_vel.data_ptr()) if cam_vel is not None else 0)
    ...
    for i in range(num_shapes):
        ...
        shapes.append(dtrt.Shape(\
            dtrt.float_ptr(vertices.data_ptr()),
            dtrt.int_ptr(indices.data_ptr()),
            dtrt.float_ptr(uvs.data_ptr() if uvs is not None else 0),
            dtrt.float_ptr(normals.data_ptr() if normals is not None else 0),
            int(vertices.shape[0]),
            int(indices.shape[0]),
            light_id, bsdf_id, med_int_id, med_ext_id, 
            dtrt.float_ptr(shape_vel.data_ptr() if shape_vel is not None else 0)))
        current_index += 9
    bsdfs = []   
    for i in range(num_bsdfs):
        if args[current_index] == 'null':
            bsdfs.append(dtrt.BSDF_null())
            current_index += 1
        elif args[current_index] == 'diffuse':
            diffuse_reflectance = args[current_index + 1]
            vec_reflectance     = dtrt.Spectrum3f(diffuse_reflectance[0], diffuse_reflectance[1], diffuse_reflectance[2])
            default             = args[current_index + 2] is None
            if default:
                bsdfs.append(dtrt.BSDF_diffuse(vec_reflectance))
            else:
                d_reflectance = args[current_index + 2]
                assert(d_reflectance.is_contiguous())
                bsdfs.append(dtrt.BSDF_diffuse(vec_reflectance, dtrt.float_ptr(d_reflectance.data_ptr())))
            current_index += 3
        elif args[current_index] == 'phong':
            diffuse_reflectance  = args[current_index + 1]
            specular_reflectance = args[current_index + 2]
            exponent = args[current_index + 3]
            vec_kd  = dtrt.Spectrum3f(diffuse_reflectance[0], diffuse_reflectance[1], diffuse_reflectance[2])
            vec_ks  = dtrt.Spectrum3f(specular_reflectance[0], specular_reflectance[1], specular_reflectance[2])
            default = args[current_index + 4] is None
            if default:
                bsdfs.append(dtrt.BSDF_Phong(vec_kd, vec_ks, exponent))
            else:
                ...
                bsdfs.append(dtrt.BSDF_Phong(vec_kd, vec_ks, exponent, 
                                                dtrt.float_ptr(d_diffuse.data_ptr()),
                                                dtrt.float_ptr(d_specular.data_ptr()), 
                                                dtrt.float_ptr(d_exponent.data_ptr())))
            current_index += 7
            ...
            default = args[current_index + 4] is None
            if default:
                bsdfs.append(dtrt.BSDF_roughdielectric(alpha, intIOR, extIOR))
            else:
                ...
                bsdfs.append(dtrt.BSDF_roughdielectric(alpha, intIOR, extIOR, dtrt.float_ptr(d_alpha.data_ptr()), dtrt.float_ptr(d_eta.data_ptr())))
            current_index += 6
        else:
            raise
    area_lights = []
    for i in range(num_lights):
        if args[current_index] == 'area_light':
            ...
            area_lights.append(dtrt.AreaLight(shape_id, dtrt.float_ptr(intensity.data_ptr()), two_sided))
            current_index += 4
        else:
            shape_id    = args[current_index + 1]
            intensity   = dtrt.Spectrum3f(args[current_index + 2][0], args[current_index + 2][1], args[current_index + 2][2])
            kappa       = args[current_index + 3]
            default     = args[current_index + 4] is None
            if default:
                area_lights.append(dtrt.AreaLightEx(shape_id, intensity, kappa))
            else:
                ...
                area_lights.append(dtrt.AreaLightEx(shape_id, intensity, kappa, dtrt.float_ptr(d_intensity.data_ptr()), dtrt.float_ptr(d_kappa.data_ptr())))
            current_index += 6
    ...
    for i in range(num_medium):
        if args[current_index] == 'homogeneous':
            ...
            vec_albedo  = dtrt.Spectrum3f(albedo[0], albedo[1], albedo[2])
            phase_id    = args[current_index + 3]
            default     = args[current_index + 4] is None
            if default:
                mediums.append(dtrt.Homogeneous(sigma_t, vec_albedo, phase_id))
            else:
                ...
                mediums.append(dtrt.Homogeneous(sigma_t, vec_albedo, phase_id, dtrt.float_ptr(d_sigmaT.data_ptr()), 
                                                                                  dtrt.float_ptr(d_albedo.data_ptr())))
            current_index += 6
            ...
            if default:
                if vol_albedo:
                    mediums.append(dtrt.Heterogeneous(fn_density, dtrt.float_ptr(to_world.data_ptr()), float(scalar), albedo, phase_id))
                else:
                    vec_albedo  = dtrt.Spectrum3f(albedo[0], albedo[1], albedo[2])
                    mediums.append(dtrt.Heterogeneous(fn_density, dtrt.float_ptr(to_world.data_ptr()), float(scalar), vec_albedo, phase_id))
            else:
                ...
                if vol_albedo:
                    mediums.append(dtrt.Heterogeneous(fn_density, dtrt.float_ptr(to_world.data_ptr()), float(scalar), albedo, phase_id,
                                                     dtrt.float_ptr(vec_translate.data_ptr()),
                                                     dtrt.float_ptr(vec_rotate.data_ptr()),
                                                     dtrt.float_ptr(d_scalar.data_ptr())))                    
                else:
                    vec_albedo  = dtrt.Spectrum3f(albedo[0], albedo[1], albedo[2])
                    mediums.append(dtrt.Heterogeneous(fn_density, dtrt.float_ptr(to_world.data_ptr()), float(scalar), vec_albedo, phase_id,
                                                     dtrt.float_ptr(vec_translate.data_ptr()),
    ...
    for i in range(num_phases):
        if args[current_index] == 'isotropic':
            phases.append(dtrt.Isotropic())
            current_index += 1
        elif args[current_index] == 'hg':
            g = args[current_index + 1]
            default  = args[current_index + 2] is None
            if default:
                phases.append(dtrt.HG(g));
            else:
                d_g  = args[current_index + 2]
                assert( d_g.is_contiguous() )
                phases.append(dtrt.HG(g, dtrt.float_ptr(d_g.data_ptr())));
            current_index += 3
        else:
            raise

    scene = dtrt.Scene(camera, shapes, bsdfs, area_lights, phases, mediums)
    if args[current_index] is not None:
        print(args[current_index])
        scene.initEdges(dtrt.float_ptr(args[current_index].data_ptr()))

    if rect[2] == -1 or rect[3] == -1:
        rendered_image = torch.zeros(dtrt.nder + 1, resolution[1], resolution[0], 3)
    else:
        rendered_image = torch.zeros(dtrt.nder + 1, rect[3], rect[2], 3)
    start = time.time()
    integrator.render(scene, options, dtrt.float_ptr(rendered_image.data_ptr()))
