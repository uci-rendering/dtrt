...
import numpy as np
import dtrt
from dtrt import nder, angleEps
import os
import pydtrt
...
def parse_camera(node, medium_dict):
    ...
    return pydtrt.Camera(position     = position,
                            look_at      = look_at,
                            up           = up,
                            fov          = fov,
                            clip_near    = clip_near,
...
def parse_bsdf(node, two_sided = False):
    ...
    if node.attrib['type'] == 'diffuse':
        ...
        return (node_id, pydtrt.BSDF_diffuse(diffuse_reflectance = diffuse_reflectance))
    elif node.attrib['type'] == 'null':
        return (node_id, pydtrt.BSDF_null())
        ...
        return (node_id, pydtrt.BSDF_Phong(diffuse_reflectance, specular_reflectance, exponent))
        ...
        return (node_id, pydtrt.BSDF_roughdielectric(alpha, intIOR, extIOR))
    else:
...
def parse_medium(node):
    ...
    if node.attrib['type'] == 'homogeneous':
        ...
        if phase == None:
            phase = pydtrt.Isotropic();
        return (node_id, pydtrt.Homogeneous(sigma_t = sigma_t, albedo = albedo, phase_id = -1), phase)
        ...
        return (node_id, pydtrt.Heterogeneous(fn_density = fn_density, albedo = albedo, scalar = scalar, to_world = to_world.contiguous(), phase_id = -1), phase)
    else:
        print('Unsupported bsdf type:', node.attrib['type'])
        assert(False)
...
def parse_shape(node, bsdf_dict, med_dict, shape_id):
    ...
    if filename == '':
        ...
        mesh_list = pydtrt.load_obj(filename)
    ...
    if light_intensity is not None:
        if light_kappa is None:
            lgt = pydtrt.AreaLight(shape_id, light_intensity)
        else:
            lgt = pydtrt.AreaLightEx(shape_id, light_intensity, light_kappa)
    return pydtrt.Shape(vertices, indices, uvs, normals, bsdf_id, med_ext_id, med_int_id), lgt
